...
- For untyped:\
  Set model to predefined gqlgen type (e.g. for int use `github.com/ssmccoy/gqlgen/graphql.Int`).
  Set enum values to specific const/var.

More examples can be found in [_examples/enum](https://github.com/ssmccoy/gqlgen/tree/master/_examples/enum).
...
enum EnumUntyped @goModel(model: "github.com/ssmccoy/gqlgen/graphql.Int") {
...
models:
  ...
  EnumUntyped:
    model: github.com/ssmccoy/gqlgen/graphql.Int
    enum_values:
      ONE:
